...
public:
    ...
    void initializeRestaurants() {
        Restaurant* restaurant1 = new Restaurant("Bikaner", "Katangi");
    ...
    void addToCart(User* user, const string& itemCode) {
        ...
        for (const auto& item : restaurant->getMenuItems()) {
            cout << "Checking item: " << item.getCode() << endl;
        ...
        cout<<user->getCart()->getItems().size()<<endl;
    }
    ...
    void printUserCart(User* user) {
        cout << "Items in cart: "<<user->getCart()->getItems().size()<<endl;
        cout << "------------------------------------" << endl;
